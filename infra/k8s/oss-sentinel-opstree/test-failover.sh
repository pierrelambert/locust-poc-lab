...
while [[ $SECONDS -lt $END ]]; do
    if kubectl exec -n "${NAMESPACE}" "${TARGET_POD}" -- redis-cli PING &>/dev/null; then
...
while [[ $SECONDS -lt $END ]]; do
    SENTINEL_POD=$(kubectl get pods -n "${NAMESPACE}" \
        -l app=redis-sentinel-sentinel \
        -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)
    if [[ -n "${SENTINEL_POD}" ]] && kubectl exec -n "${NAMESPACE}" "${SENTINEL_POD}" -- \
